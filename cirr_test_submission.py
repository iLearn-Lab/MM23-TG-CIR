...
def test_cirr_submit_result(model, testset, save_dir, batch_size = 16):
    ...
    with open(os.path.join(save_dir, 'CIRR_pred_ranks_recall.json'), 'w+') as f:
        json.dump(general_submission, f, sort_keys=True)
        
    with open(os.path.join(save_dir, 'CIRR_pred_ranks_recall_subset.json'), 'w+') as f:
        json.dump(subset_submission, f, sort_keys=True)

if __name__ == '__main__':
    _, preprocess = clip.load('ViT-B/16', device='cpu', jit=False)
    testset = datasets.CIRR(path=...,transform=preprocess)
    model_dir = ...
    # load model
    model = torch.load(os.path.join(model_dir, 'CIRR.pt'))
    # generate submission json file
    test_cirr_submit_result(model, save_dir=model_dir, testset=testset, batch_size=128)